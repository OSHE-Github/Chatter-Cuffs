...
#else
const uint8_t BUILT_IN_LED = GPIO_NUM_13;  // Use GPIO 13 instead of GPIO 2
#endif
...
// For generic ESP32 boards, we can use different flash patterns to distinguish states
void GenericDevBoardIndicatorLed::set_led_rgb(uint32_t color)
{
  if (color == 0)
  {
    digitalWrite(BUILT_IN_LED, LOW);  // LED OFF
  }
  else
  {
    digitalWrite(BUILT_IN_LED, HIGH); // LED ON (any color)
  }
}
