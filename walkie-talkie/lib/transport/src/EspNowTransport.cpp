...
void EspNowTransport::change_channel(uint8_t new_channel)
{
  Serial.printf("ESP-NOW: Changing channel from %d to %d\n", m_wifi_channel, new_channel);
  ...
  if (result == ESP_OK)
  {
    Serial.printf("ESP-NOW: Successfully changed to channel %d\n", m_wifi_channel);
    esp_now_register_recv_cb(receiveCallback);
    
    // Re-add the broadcast peer
    esp_now_peer_info_t peerInfo = {};
    memcpy(&peerInfo.peer_addr, broadcastAddress, 6);
    if (!esp_now_is_peer_exist(broadcastAddress))
    {
      ...
      if (result != ESP_OK)
      {
        Serial.printf("Failed to re-add broadcast peer: %s\n", esp_err_to_name(result));
      }
    }
  }
  else
  {
    Serial.printf("ESP-NOW: Failed to reinitialize on channel %d: %s\n", m_wifi_channel, esp_err_to_name(result));
  }
}